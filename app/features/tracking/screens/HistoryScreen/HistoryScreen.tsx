...
/**
 * History screen showing all tracked BFRB instances with a modern UI
 */
export default function HistoryScreen({ visible, onClose }: { visible: boolean, onClose: () => void }) {
  // State management
  ...
  useEffect(() => {
    console.log('HistoryScreen: Auth status:', isAuthenticated);
    if (user) {
      console.log('HistoryScreen: User:', user.displayName, user.email);
    }
  ...
  const fetchInstances = async () => {
    try {
      ...
      if (!isAuthenticated) {
        console.log('HistoryScreen: Not authenticated, skipping fetch');
        setLoading(false);
        setRefreshing(false);
        return;
      }
      
      console.log('HistoryScreen: Fetching instances...');
      ...
      if (!Array.isArray(response)) {
        ...
        console.error('HistoryScreen: Invalid response format:', response);
        setError(errorMessage);
        return;
      }
      
      console.log(`HistoryScreen: Fetched ${response.length} instances`);
      ...
      console.error('HistoryScreen: Error fetching instances:', err);
      
      let errorMessage = 'Failed to load your history data. Please try again.';
  ...
  const handleExportCSV = async () => {
    ...
    try {
      console.log('HistoryScreen: Exporting instances to CSV...');
      await exportInstancesAsCSV(instances);
      console.log('HistoryScreen: CSV export completed');
    } catch (err) {
      console.error('HistoryScreen: Error exporting to CSV:', err);
    } finally {
      setExportLoading(false);
    }
  };

  // Load instances when the screen becomes visible
  useEffect(() => {
  ...
  const viewInstanceDetails = (instance: Instance) => {
    try {
      console.log('HistoryScreen: Opening details for instance:', instance._id);
      setSelectedInstanceId(instance._id);
      setDetailsModalVisible(true);
    } catch (err) {
      console.error('HistoryScreen: Error viewing details:', err);
    }
  };
  ...
  const closeDetailsModal = () => {
    console.log('HistoryScreen: Closing details modal');
  ...
  const EmptyState = () => (
    <View style={styles.emptyState}>
      ...
      <Text style={styles.emptyStateTitle}>No history data yet</Text>
        ...
        <SafeAreaView style={styles.container}>
          <View style={styles.header}>
            <TouchableOpacity 
              ...
              accessibilityHint="Double tap to return to insights"
            >
          ...
          {loading && initialLoad ? (
            <View style={styles.loadingContainer}>
              ...
              <Text style={styles.loadingText}>Loading your history data...</Text>
            </View>
          ) : (
            <FlatList
              data={instances}
